...
function profile(user) {
  return { id: user.id, email: user.email, displayName: user.display_name, avatarUrl: user.avatar_url, createdAt: user.created_at };
}

export async function onRequestGet({ request, env }) {
  const auth = await requireUser(request, env);
  if (auth.error) return accountError(request, auth.error, auth.status);
  const settings = await auth.db.prepare('SELECT notifications_enabled, alert_lead_minutes, sunrise_alerts, sunset_alerts, alert_threshold FROM user_settings WHERE user_id = ?').bind(auth.user.id).first();
  return accountJson(request, { user: profile(auth.user), settings: settings || null });
}
